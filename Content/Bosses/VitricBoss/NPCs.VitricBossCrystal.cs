...
namespace StarlightRiver.Content.Bosses.VitricBoss
{
	internal class VitricBossCrystal : ModNPC, IDrawAdditive
	{
		...
		public static readonly Asset<Texture2D> texture_AssetDirectory_VitricBoss___FinalLaser = ModContent.Request<Texture2D>(AssetDirectory.VitricBoss + "FinalLaser");
		...
		public static readonly Asset<Texture2D> texture_AssetDirectory_VitricBoss___GlassSpikeGlow = ModContent.Request<Texture2D>(AssetDirectory.VitricBoss + "GlassSpikeGlow");
		...
		public static readonly Asset<Texture2D> texture_AssetDirectory_VitricBoss___VitricBossCrystalShape = ModContent.Request<Texture2D>(AssetDirectory.VitricBoss + "VitricBossCrystalShape");
		public static readonly Asset<Texture2D> texture_AssetDirectory_VitricBoss___VitricBossCrystalGlowBlue = ModContent.Request<Texture2D>(AssetDirectory.VitricBoss + "VitricBossCrystalGlowBlue");
		public static readonly Asset<Texture2D> texture_AssetDirectory_VitricBoss___VitricBossCrystalGlowOrange = ModContent.Request<Texture2D>(AssetDirectory.VitricBoss + "VitricBossCrystalGlowOrange");
		...
		{
			spriteBatch.Draw(texture_Texture.Value, NPC.Center - screenPos, NPC.frame, drawColor, NPC.rotation, NPC.frame.Size() / 2, NPC.scale, 0, 0);
			return false;
		}

		public override void PostDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
		{
			Texture2D tex = texture_Texture___Glow.Value; //glowy outline
			if (state == 0)
			...
			if (phase == 3 && timer < 30)
			{
				...
				spriteBatch.Draw(texture_Texture.Value, NPC.Center - screenPos + new Vector2(2, 0), NPC.frame, Color.White * (1 - factor), NPC.rotation, NPC.frame.Size() / 2, factor * 2 * NPC.scale, 0, 0);
			}

			spriteBatch.Draw(texture_AssetDirectory_VitricBoss___VitricBossCrystalGlowOrange.Value, NPC.Center - screenPos, NPC.frame, Color.White * 0.8f, NPC.rotation, NPC.frame.Size() / 2, NPC.scale, 0, 0);
			spriteBatch.Draw(texture_AssetDirectory_VitricBoss___VitricBossCrystalGlowBlue.Value, NPC.Center - screenPos, NPC.frame, Color.White * 0.6f, NPC.rotation, NPC.frame.Size() / 2, NPC.scale, 0, 0);

			if (phase >= 5)
			{
				spriteBatch.Draw(texture_AssetDirectory_VitricBoss___VitricBossCrystalShape.Value, NPC.Center - screenPos, NPC.frame, Color.White * (timer / 120f), NPC.rotation, NPC.frame.Size() / 2, NPC.scale, 0, 0);
			}
		}
		...
		public void DrawAdditive(SpriteBatch spriteBatch)
		{
			...
			if (phase == 3)
			{
				Texture2D tex = texture_AssetDirectory_VitricBoss___GlassSpikeGlow.Value;
			...
			{
				Texture2D texGlow2 = texture_StarlightRiver_Assets_Keys_Glow.Value;
				Texture2D ballTex = texture_AssetDirectory_VitricBoss___FinalLaser.Value;
			...
			if (Parent != null && Parent.Phase == (float)AIStates.FirstPhase && Parent.AttackPhase == 1) //total bodge, these should draw on every crystal not just the oens that draw arcs. this detects the attack on the parent
			{
				if (Parent.AttackTimer > 360)
				{
					...
					Texture2D tex2 = texture_Texture___Outline.Value;
