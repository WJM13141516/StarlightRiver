...
namespace StarlightRiver.Content.Items.Magnet
{
	public class ThunderBeads : ModItem
	{
		...
		public static readonly Asset<Texture2D> texture_AssetDirectory_Keys___Glow = ModContent.Request<Texture2D>(AssetDirectory.Keys + "Glow");
	...
	public class ThunderBeadsProj : BaseWhip
	{
		...
		public override void DrawBehindWhip(ref Color lightColor)
		{
			...
			if (embedded)
			{
				...
				Texture2D bloomTex = texture_AssetDirectory_Keys___Glow.Value;
				for (int i = 0; i < cache.Count - 1; i++)
				{
					if (i % 3 != 0)
						continue;
